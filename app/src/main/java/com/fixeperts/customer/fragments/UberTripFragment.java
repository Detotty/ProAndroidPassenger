...
import com.fixeperts.customer.component.MyFontPopUpTextView;
import com.fixeperts.customer.component.MyFontTextView;
...
import com.fixeperts.customer.parse.HttpRequester;
...
import com.fixeperts.customer.utils.MathUtils;
	...
	public void onClick(View v) {
		...
		case R.id.layoutCard:
			if (layoutCard.isSelected()) {
				startActivity(new Intent(getActivity(),UberViewPaymentActivity.class));
			} else {
				layoutCash.setSelected(false);
				layoutCard.setSelected(true);
				tvCardNo.setTextColor(getResources().getColor(R.color.white));
...
//		animateCameraToMarkerWithZoom(latLngDriver);
	...
	public void onTaskCompleted(final String response, int serviceCode) {
		...
		case Const.ServiceCode.GET_DURATION:
			...
			myMarker.setIcon(BitmapDescriptorFactory.fromBitmap(AndyUtils.createDrawableFromView(getActivity(), markerLayout)));
		...
		case Const.ServiceCode.PAYMENT_TYPE:
			...
			if (!activity.pContent.isSuccess(response)) {
				AndyUtils.showToast("Sorry, Cannot change payment mode",getActivity());
			}
			break;
