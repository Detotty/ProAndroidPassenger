...
import com.fixeperts.customer.UberViewPaymentActivity;
import com.fixeperts.customer.adapter.PlacesAutoCompleteAdapter;
import com.fixeperts.customer.adapter.VehicalTypeListAdapter;
import com.fixeperts.customer.component.MyFontButton;
import com.fixeperts.customer.interfaces.OnProgressCancelListener;
import com.fixeperts.customer.models.Driver;
import com.fixeperts.customer.models.Route;
import com.fixeperts.customer.models.Step;
import com.fixeperts.customer.models.VehicalType;
import com.fixeperts.customer.parse.ParseContent;
...
import com.fixeperts.customer.utils.LocationHelper;
import com.fixeperts.customer.utils.PreferenceHelper;
import com.fixeperts.customer.utils.LocationHelper.OnLocationReceived;
...
public class UberMapFragment extends UberBaseFragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		//speech button
		spchBtn=(ImageButton) view.findViewById(R.id.speechBtn);
		
		layoutCardDetails = (LinearLayout) view.findViewById(R.id.layoutCardDetails);
		...
		btnSelectService = (MyFontButton) view.findViewById(R.id.btnSelectService);
	...
	private void pickMeUp() {
		...
		if (markerDestination != null) {
			...
			map.put(Const.Params.DESTI_LONGITUDE,String.valueOf(dest.longitude));
		}
		// new HttpRequester(activity, map, Const.ServiceCode.CREATE_REQUEST,
		// this);
		requestQueue.add(new VolleyHttpRequest(Method.POST, map, Const.ServiceCode.CREATE_REQUEST, this, this));
	}
	...
	public void onTaskCompleted(final String response, int serviceCode) {
		...
		case Const.ServiceCode.CREATE_REQUEST:
			...
			if (activity.pContent.isSuccess(response)) {
				...
				activity.pHelper.putRequestId(activity.pContent.getRequestId(response));
				Driver driverInfo = activity.pContent.getDriverDetail(response);
				driverId = driverInfo.getDriverId();
				AndyUtils.showCustomProgressDialog(activity,getString(R.string.text_contacting), false, this,
						driverInfo);
				stopUpdateProvidersLoaction();
				startCheckingStatusUpdate();
			}
				...
				if (new JSONObject(response).getBoolean("success")) {
					new Thread(new Runnable() {
						...
						public void run() {
							...
							listDriver = activity.pContent.parseNearestDrivers(response);
	...
	private void setProvirderOnMap() {
		...
		for (int i = 0; i < listDriver.size(); i++) {
			...
			if (vehicle.getId() == driver.getVehicleTypeId()) {
				...
				nearDriverMarker.put(driver.getDriverId(), m);
				mapMarker.add(m);
			}
		}
	...
	private void updateProviderOnMap() {
		try {
			...
			for (int i = 0; i < listUpdatedDriver.size(); i++) {
				...
				if (vehicle.getId() == driver.getVehicleTypeId()) {
					isGetProvider = true;
					getDuration(curretLatLng,String.valueOf(driver.getLatitude()),String.valueOf(driver.getLongitude()));
					break;
				}
			}
			listDriver.clear();
	...
	private String getFareCalculation(double distanceInKm) {
		...
		double distanceCost = vehicle.getPricePerUnitDistance();
		 if (vehicle.getUnit().equals(getString(R.string.miles))) {

				distanceInKm = distanceInKm * 0.621371;
				Log.i("distanceinmile", distanceInKm + "");
			}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case 65636:
			if (resultCode == getActivity().RESULT_OK && null != data) {
				...
				Toast.makeText(getActivity(), talk, Toast.LENGTH_LONG).show();
				...
				if((talk.contains("driver near to my location") )|| 
						...
						(talk.contains("find the closest driver"))){
					       ...
					       findTheNearestDriver(null,talk);
				}
				...
				{
					Toast.makeText(getActivity(), talk.substring(15), Toast.LENGTH_LONG).show();
					
					findTheNearestDriver(talk.substring(15).trim(),talk);
				}
			}
			break;
		}
	...
	// fuction calling from onActivityResult for nearby taxi
	public void findTheNearestDriver(String loc,String talk){

		try {
			Location my=new Location("My");

			if(loc==null){
				my.setLatitude(curretLatLng.latitude); 
				my.setLongitude(curretLatLng.longitude);
			}
			...
			Marker m1=mapMarker.get(min);
			Toast.makeText(getActivity(), talk, Toast.LENGTH_LONG).show();
			onMarkerClick(m1,minDis);
	...
	public boolean onMarkerClick(Marker marker,float mindist) {
		try {
			//Toast.makeText(getActivity(),"Loading vehicle information..", Toast.LENGTH_SHORT).show();
			Integer Driverid=0;
			
			int driverid = 0;
			double lat = 0.0,longg = 0.0, rating= 0;
			float dist = mindist; 
			String bio = "", bill = "",carmodel= "",fname = "",lname= "",lasttime="",phone= "",pic = "";
			
			Map<Integer, Marker> testMap = new HashMap<Integer, Marker>();
			 
			for (Entry<Integer, Marker> entry : nearDriverMarker.entrySet()) {
			        if (entry.getValue().equals(marker)) {
			            Driverid = entry.getKey();
			        }
			    }
			
			VehicalType vehiclex = listType.get(selectedPostion);
			
			for (int j = 0; j < listDriver.size(); j++) {
				Driver driver = listDriver.get(j);
				
				if (driver.getDriverId() == Driverid) {
					
					
					bio = driver.getBio();
					//bill = driver.getBill().getBasePrice();
					carmodel = driver.getCarModel()+", "+driver.getCarNumber();
					driverid = driver.getDriverId();
				    fname = driver.getFirstName();
					lname = driver.getLastName();
					lasttime = driver.getLastTime();
					lat = driver.getLatitude();
					longg = driver.getLongitude();
					phone = driver.getPhone();
					pic = driver.getPicture();
					rating = driver.getRating();
					
				}
			}
			
			showDriverDetailsWindow(marker, bio,bill,carmodel,driverid,fname,lname,lasttime, lat,longg,phone,pic,rating,dist);
	...
	public void showDriverDetailsWindow(Marker mark, String bio, String bill, String carmodel, int driverid, String fname,
			String lname, String lasttime, Double lat, Double longg, String phone, String pic, Double rating, float dist){
		...
		try {
			...
			TextView driver_id=(TextView) dialog.findViewById(R.id.driverId);
			TextView drivername=(TextView) dialog.findViewById(R.id.driverName);
			TextView drivernumber=(TextView) dialog.findViewById(R.id.driverPhone);
			TextView driverbio=(TextView) dialog.findViewById(R.id.bio);
			TextView driverpic=(TextView) dialog.findViewById(R.id.driverPic);
			driverpic.setVisibility(View.GONE);
			TextView driverrating=(TextView) dialog.findViewById(R.id.driverRating);
			TextView drivelasttime=(TextView) dialog.findViewById(R.id.driverLasttime);
			drivelasttime.setVisibility(View.GONE);
			...
			title.setText("Taxi Details"+" | "+diststr+" Away");
			
			
			driver_id.setText(driverid+"");
			drivername.setText(fname+" "+lname);
			drivernumber.setText(phone);
			driverbio.setText(bio);
			driverpic.setText(vehicle.getBasePrice()+"");
			driverrating.setText(rating+"");
			//drivelasttime.setText(lasttime);
			vehicleInfo.setText(carmodel);
			costkm.setText(vehicle.getPricePerUnitDistance()+" $");
			loc.setText(lat+", "+longg);
			
			/*if (lat != null) {
				LatLng latLangnear = new LatLng(lat,longg);
				animateCameraToMarker(latLangnear, true);
			}*/
