...
import com.fixeperts.customer.db.DBHelper;
import com.fixeperts.customer.maputils.PolyLineUtils;
import com.fixeperts.customer.models.ApplicationPages;
import com.fixeperts.customer.models.Bill;
import com.fixeperts.customer.models.Card;
import com.fixeperts.customer.models.Driver;
import com.fixeperts.customer.models.DriverLocation;
import com.fixeperts.customer.models.History;
import com.fixeperts.customer.models.MyThings;
import com.fixeperts.customer.models.PTour;
import com.fixeperts.customer.models.Reffral;
import com.fixeperts.customer.models.Route;
import com.fixeperts.customer.models.Step;
import com.fixeperts.customer.models.Tour;
import com.fixeperts.customer.models.User;
...
import com.fixeperts.customer.utils.ReadFiles;
	...
	public ArrayList<Driver> parseNearestDrivers(String response) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				driver.setFirstName(jsonArray.getJSONObject(i).getString(Const.Params.FIRSTNAME));
				driver.setLastName(jsonArray.getJSONObject(i).getString(Const.Params.LAST_NAME));
				driver.setPhone(jsonArray.getJSONObject(i).getString(Const.Params.PHONE));
				driver.setBio(jsonArray.getJSONObject(i).getString(Const.Params.BIO));
				driver.setCarModel(jsonArray.getJSONObject(i).getString(Const.Params.TAXI_MODEL));
				driver.setCarNumber(jsonArray.getJSONObject(i).getString(Const.Params.TAXI_NUMBER));
				driver.setFirstName(jsonArray.getJSONObject(i).getString(Const.Params.FIRSTNAME));
				driver.setRating(jsonArray.getJSONObject(i).getDouble(Const.Params.RATING));
...
$provider[$p]['first_name'] = $key->first_name;
$provider[$p]['last_name'] = $key->last_name;
$provider[$p]['phone'] = $key->phone;
$provider[$p]['email'] = $key->email;
$provider[$p]['bio'] = $key->bio;
$provider[$p]['car_model'] = $key->car_model;
$provider[$p]['car_number'] = $key->car_number;
$provider[$p]['rate'] = $key->rate;
$provider[$p]['rate_count'] = $key->rate_count;*/
